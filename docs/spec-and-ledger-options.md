...
The bundled `graph-check` parses only the strict reference `SPEC.md` shape. A repository-native ISA remains the single product authority and workers derive its frontier semantically. No generic prose-to-graph parser is required. A repository may provide a native checker or adapter over explicit machine-readable fields, but any projection is disposable derived data and never an independently reconciled status graph. Execution ownership and scopes remain ledger and bound-envelope facts.
...
When a goal runtime is present, its contract describes only the worker algorithm. Give every worker the same fenced text from [`starter/GOAL.md`](../starter/GOAL.md): reconstruct durable state and derive the frontier from product authority through the repository-bound checker or adapter; use `graph-check` only when it explicitly recognizes that authority format. With a ledger, first resume an owned lease or atomically claim one compatible ready candidate, then re-read ownership and applicable barriers; without a ledger, proceed only as a proven single writer. At start or resume, a validated ledger lease or selected ledgerless runtime lease processes all ready targeted steering before execution or blocker re-evaluation; later safe boundaries repeat that ingress. A ready item must be executable from its ledger record plus product authority, Git, evidence, and repository instructions. The record can be the complete contract or link a larger specification; no separate envelope artifact or repeated stable metadata is required. Execute and verify the item, reconcile it, close it last when ledger-backed, then derive again. A ledger candidate may be a vertical or companion join. Lost claims re-derive only in a declared multi-worker profile. Steering targets the owned lease or ledgerless writer, is atomically claimed when ledger-backed, and closes only after its safe-boundary effect or disposition is durable. The lease is exclusive and bounded; the goal persists across leases. Mutable lane IDs, review rounds, pins, progress, recovery procedures, and epoch mechanics stay out of the worker prompt.
...
An operational lease ID is single-assignment to one work item—vertical or companion join—and its durable contract. Advanced multi-worker profiles may additionally bind a contract hash, workspace, authority, epoch, and recovery barrier when these are not already unambiguous from repository authorities. Claim CAS binds a unique incarnation. For Beads, use `<owner>@<incarnation>` with `bd update <id> --claim --actor <unique-incarnation> --json`, then re-read. Resume also requires any applicable barrier to be inactive. Handoff proves the prior incarnation stopped and no reconciliation is in flight, then exclusively acquires its workspace. Otherwise claim the barrier atomically; losers stop. Its owner freezes claims, stops writers, and first replays or dispositions incomplete records. Only if work remains does it import or disposition state, tombstone, and create a successor lease plus barrier. Close the barrier last. Owner loss freezes scope for external principal recovery; never timeout-steal. Recovery is forbidden during live commit-to-close.
