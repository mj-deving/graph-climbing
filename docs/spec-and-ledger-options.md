...
When a goal runtime is present, its contract describes only the worker algorithm. Give every worker the same fenced text from [`starter/GOAL.md`](../starter/GOAL.md): reconstruct durable state and derive the frontier; with a ledger, first validate and resume a lease already owned by the same runtime incarnation, otherwise validate and atomically claim one compatible ready candidate, then re-read exact ownership and bindings; without a ledger, proceed only as a proven single writer and locally select one bounded frontier item as the runtime lease. Every executable item supplies a complete immutable envelope; ownership, barrier, and epoch controls are conditional on a ledger. Execute and verify that lease, reconcile it, close it last when ledger-backed, then derive again. A ledger candidate may be a vertical or companion join. Lost claims re-derive only in a declared multi-worker profile. Steering targets the owned lease or ledgerless writer, is atomically claimed when ledger-backed, and closes only after its safe-boundary effect or disposition is durable. The lease is exclusive and bounded; the goal persists across leases. Mutable lane IDs, review rounds, pins, progress, recovery procedures, and epoch mechanics stay out of the worker prompt.
