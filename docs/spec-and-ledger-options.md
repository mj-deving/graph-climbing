...
An operational lease ID is single-assignment to one stable vertical and canonical release-envelope hash. Only the declared reconciliation owner in its exclusive authority workspace writes official product truth. Recovery or reassignment is forbidden during its commit-to-close critical section. Otherwise recovery durably stops every writer for that authority scope, freezes claims, inspects Git and incomplete records, tombstones the old lease, and creates a new one. Tombstoned records become superseded. This prevents stale writes and replay without requiring a cross-store transaction the tools do not provide.
