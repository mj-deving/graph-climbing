...
An operational lease ID is single-assignment to one stable vertical, canonical envelope hash, workspace, applicable epoch, and pre-created authority-scope recovery barrier ID. The release owner pre-binds those fields before Ready; claim CAS binds a unique incarnation. For Beads, use `<owner>@<incarnation>` with `bd update <id> --claim --actor <unique-incarnation> --json`, then re-read. Handoff proves the prior incarnation stopped and no reconciliation is in flight, then exclusively acquires its workspace. Otherwise atomically claim the recovery barrier; losers stop. Its owner freezes claims, stops every scope writer, then first replays or dispositions incomplete records on active leases. Replay closes without replacement. Only if work remains does it import or disposition all state, tombstone the lease, and create a new pre-bound lease plus successor barrier. Close the barrier last. Recovery is forbidden during a live commit-to-close.
