...
When a goal runtime is present, its contract describes only the worker algorithm. Give every worker the same fenced text from [`starter/GOAL.md`](../starter/GOAL.md): reconstruct durable state and derive the frontier; with a ledger, validate one compatible ready candidate and its envelope plus any applicable epoch and inactive barriers, claim it atomically with a unique runtime incarnation, and re-read exact ownership and bindings; without a ledger, proceed only as a proven single writer and locally select one bounded frontier item as the runtime lease. Execute and verify that lease, reconcile it, close it last when ledger-backed, then derive again. A ledger candidate may be a vertical or companion join. Lost claims re-derive only in a declared multi-worker profile; steering applies at safe boundaries and persists durably. The lease is exclusive and bounded; the goal persists across leases. Mutable lane IDs, review rounds, pins, progress, recovery procedures, and epoch mechanics stay out of the worker prompt.
