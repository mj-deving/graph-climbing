...
N mutating workers require one atomic ledger even though a proven single-writer adoption does not. One central reconciliation owner publishes the current admission epoch with graph/product base, candidate IDs, immutable envelope hashes, and proof that every pair is compatible with all active leases. It may replace the epoch only when the previous epoch has no claimed, active, or sealed lanes and its joins are reconciled or withdrawn. Workers verify the same epoch immediately before and after claim; a mismatch causes release without mutation. Candidate selection uses ledger priority plus a stable-ID tie-break. Claim losers refresh; abandoned leases require explicit authorized recovery. A ready join is claimable only with declared integration and checkout authority.
