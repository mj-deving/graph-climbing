...
An operational lease ID is single-assignment to one stable vertical and canonical release-envelope hash. Recovery first fences the old worker, tombstones that lease ID, and creates a new lease ID. The reconciliation record binds lease ID, vertical ID, envelope hash, and run epoch when present; exact-ID close happens last. This avoids stale replay closing a replacement without requiring a compare-and-swap feature the ledger may not provide.
