...
N mutating workers require one atomic ledger even though a proven single-writer adoption does not. Before release, the graph validates that every pair of simultaneously claimable envelopes is compatible with all active leases; their scopes then stay immutable. Candidate selection uses ledger priority plus a stable-ID tie-break. Claim losers refresh and try the next candidate; abandoned leases require explicit authorized recovery. Every release envelope carries the exact base, checkout policy, claim closure, dependency and scope boundaries, probe, stop rule, evidence destination, product-authority write permission, admission evidence, and companion join. A ready join is just another graph-derived lease, but only a worker with declared integration and checkout authority may claim it.
