...
An operational lease ID is single-assignment to one stable vertical, canonical release-envelope hash, workspace, and epoch. The release owner pre-binds those fields before Ready; the claim CAS binds a unique runtime incarnation. For Beads, use a unique actor such as `<owner>@<incarnation>` with `bd update <id> --claim --actor <unique-incarnation> --json`, then re-read the lease. Resume requires exact bindings. Intentional handoff proves the prior incarnation stopped and no reconciliation is in flight, then exclusively acquires its original workspace or durably transfers/dispositions all Git, filesystem, runtime, and evidence state before rebinding; uncertainty uses recovery. Only the declared reconciliation owner writes official product truth. Recovery is forbidden during commit-to-close and otherwise stops every writer for that authority scope, resolves state, tombstones the old lease, and creates a new pre-bound ID.
