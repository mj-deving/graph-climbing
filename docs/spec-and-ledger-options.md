...
N mutating workers require one atomic ledger even though a proven single-writer adoption does not. Initial activation and replacement share a clean barrier: pause or terminate every mutating worker, reconcile or withdraw every prior lease, seal, reservation, and join, then let one central owner publish an immutable run epoch. It binds graph/product base, execution-graph revision, candidate IDs, canonical envelope hashes, exclusive workspaces, and proof that every pair whose reservation lifetimes can overlap is compatible. These immutable fields are persisted before a lease becomes Ready. A reservation runs from claim through direct reconciliation or cohort join. Atomic claim binds the unique runtime incarnation as actor/assignee; workers immediately re-read every pre-bound field. They must not assume claim-time metadata flags share the claim compare-and-swap unless the installed ledger proves it. Mismatch permits no mutation. The unchanged goals resume afterward.
...
An operational lease ID is single-assignment to one stable vertical, canonical release-envelope hash, workspace, and epoch when applicable. The release owner pre-binds those fields before Ready; the claim CAS binds a unique runtime incarnation. For Beads, use a unique actor such as `<owner>@<incarnation>` with `bd update <id> --claim --actor <unique-incarnation> --json`, then re-read the lease. Resume requires exact bindings. Intentional handoff proves the prior incarnation stopped and no reconciliation is in flight, then exclusively acquires its original workspace before rebinding. An unavailable workspace or any state transfer uses recovery. Only the declared reconciliation owner writes official product truth. Recovery is forbidden during commit-to-close and otherwise stops every writer and freezes claims for that authority scope. It first replays or dispositions incomplete reconciliation records while their exact leases remain active. Successful replay closes without replacement. Only if work remains does recovery resolve and durably import or disposition all state, tombstone the old lease, and create a new pre-bound ID.
