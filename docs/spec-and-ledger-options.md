...
N mutating workers require one atomic ledger even though a proven single-writer adoption does not. Before workers start, one central reconciliation owner publishes an immutable run epoch with graph/product base, execution-graph revision, candidate IDs, canonical envelope hashes, and proof that every pair that can become concurrent is compatible. Workers verify both epoch and hash immediately before and after claim; mismatch causes release without mutation. Epoch replacement is not a live operation: pause or terminate every epoch worker, reconcile or withdraw all lanes and joins, publish the replacement, then resume the unchanged goals. Candidate selection uses ledger priority plus a stable-ID tie-break. Claim losers refresh; abandoned leases require explicit authorized recovery.
