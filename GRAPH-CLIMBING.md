...
The worker goal states policy, not the coordination implementation. The ledger and repository authorities must make invalid candidates unavailable and every ready item reconstructibly executable. A self-contained ledger record is enough; it may link a larger specification when useful. Recovery or epoch changes remain behind explicit barriers. If durable authorities are insufficient, keep the item blocked or use bounded one-shot execution rather than enlarging the prompt. The distributable contract and system boundary are in [`starter/GOAL.md`](starter/GOAL.md).
