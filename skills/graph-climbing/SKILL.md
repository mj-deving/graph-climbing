...
- Resolve `serial-no-ledger`, `serial-ledger`, or `N-worker-epoch` before work. The release owner pre-binds vertical, envelope hash, workspace, and applicable epoch before Ready; atomic claim binds a unique incarnation as actor/assignee. Resume exact bindings only. Handoff proves the prior incarnation stopped, no reconciliation is in flight, then exclusively acquires its workspace. Otherwise recovery stops every authority-scope writer, freezes claims, and first replays or dispositions incomplete reconciliation records on their active leases. Successful replay closes without replacement. Only if work remains does recovery resolve and import or disposition all state, tombstone the old lease, and issue a new pre-bound lease. Steering closes after its effect or replacement record is durable. Only the reconciliation owner writes truth. Recovery is forbidden during commit-to-close.
