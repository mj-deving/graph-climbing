...
- Keep worker and operator responsibilities separate. Ledger workers validate one ready compatible candidate—vertical or companion join—plus its envelope, any applicable epoch, and inactive barriers; claim it atomically with a unique runtime incarnation; then re-read exact ownership, bindings, and barriers. A lost claim re-derives only in a declared multi-worker run; otherwise it disproves the profile. A proven ledgerless single writer locally selects one bounded frontier item as its runtime lease. Workers execute, verify, reconcile, and close the exact lease last; steering applies only at safe boundaries and persists durably. Release owners pre-bind and validate envelopes before Ready. Recovery, handoff, and epoch replacement are operator procedures behind explicit barriers; ordinary workers stop on mismatch rather than reconstructing those protocols from the goal prompt.
