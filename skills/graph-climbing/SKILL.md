...
- Resolve `serial-no-ledger`, `serial-ledger`, or `N-worker-epoch` before work. The release owner pre-binds vertical, envelope hash, workspace, and applicable epoch before Ready; atomic claim binds a unique incarnation as actor/assignee. Resume exact bindings only. Handoff proves the prior incarnation stopped, then exclusively acquires its workspace or durably transfers and dispositions all Git, filesystem, runtime, and evidence state; uncertainty recovers. Steering closes after its effect or replacement record is durable. Only the reconciliation owner writes truth. Recovery is forbidden during commit-to-close; otherwise stop every authority-scope writer and freeze claims before resolving, tombstoning, and reissuing.
