...
- Resolve `serial-no-ledger`, `serial-ledger`, or `N-worker-epoch` before work. The release owner pre-binds vertical, envelope hash, workspace, and epoch before Ready; atomic claim binds a unique incarnation as actor/assignee. Resume exact bindings only. Handoff preserves or dispositions all workspace state; uncertainty recovers. Steering closes after its effect or replacement record is durable. Only the reconciliation owner writes truth; recovery is forbidden during commit-to-close.
