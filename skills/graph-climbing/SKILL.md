...
- Resolve `serial-no-ledger`, `serial-ledger`, or `N-worker-epoch` before work. Every ledger lease binds a canonical envelope hash; only N-worker leases also bind an epoch. A serial claim race disproves its profile, while N-worker claim losers refresh. A ledgerless single writer reconstructs from product truth, Git, and evidence. For N writers, one central owner publishes an immutable run epoch after a clean barrier; every pair whose reservations can overlap must be compatible. Reconcile separate stores through an idempotent record; close the ledger last.
