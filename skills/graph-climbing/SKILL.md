...
- Resume an existing owned lease before selection. For N writers, release only a validated frontier whose simultaneously claimable envelopes are pairwise compatible and immutable. Reconcile separate stores through an idempotent record; close the ledger last.
