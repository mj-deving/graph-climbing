...
- Keep worker and operator responsibilities separate. Every executable item must be reconstructible from its ledger record plus product authority, Git, evidence, and repository instructions. The record may hold its contract inline or link a larger specification; do not require a second envelope artifact or repeat stable defaults. Ledger workers first validate and resume a lease already owned by the same runtime incarnation; otherwise they claim one compatible ready candidate—vertical or companion join—and re-read ownership and applicable barriers. A lost claim re-derives only in a declared multi-worker run; otherwise it disproves the profile. A proven ledgerless single writer selects one bounded reconstructible frontier item. Workers derive unambiguous details, execute, verify, reconcile, and close the exact lease last. They stop only when missing context could change outcome, authority, ownership, safety, or permitted side effects. Steering targets the owned lease or ledgerless writer, uses an atomic claim when ledger-backed, and closes only after its safe-boundary effect or disposition is durable. Recovery, handoff, and epoch replacement are operator procedures behind explicit barriers.
