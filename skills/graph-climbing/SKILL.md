...
- Resolve `serial-no-ledger`, `serial-ledger`, or `N-worker-epoch` before work. Every ledger lease ID is single-assignment to one vertical and envelope hash; recovery tombstones it and creates a new lease ID. Only N-worker leases also bind an epoch. A serial claim race disproves its profile, while N-worker claim losers refresh. For N writers, one central owner publishes an immutable run epoch after a clean barrier. Reconcile separate stores through an idempotent `{lease_id, envelope_hash, epoch}` record and close that exact ledger ID last.
