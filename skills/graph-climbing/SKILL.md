...
- Resolve `serial-no-ledger`, `serial-ledger`, or `N-worker-epoch` before work. Only N-worker ledger leases require epoch/hash binding; serial ledger leases still use exact atomic claims. A ledgerless single writer reconstructs from product truth, Git, and evidence. For N writers, one central owner publishes an immutable run epoch after a clean barrier; every pair whose reservations can overlap must be compatible. Reconcile separate stores through an idempotent record; close the ledger last.
