...
- Resolve `serial-no-ledger`, `serial-ledger`, or `N-worker-epoch` before work. Every ledger lease binds one vertical, envelope hash, worker incarnation, and workspace. Resume exact bindings only. Handoff proves the prior incarnation stopped and preserves the original workspace or durably transfers/dispositions all local state; otherwise recover. Steering closes only after its effect or replacement record is durable. Only the declared reconciliation owner writes product truth.
