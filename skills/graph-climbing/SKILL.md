...
- Resolve `serial-no-ledger`, `serial-ledger`, or `N-worker-epoch` before work. Every ledger lease binds one vertical, envelope hash, unique worker incarnation, and exclusive workspace. Resume exact bindings only; handoff proves the prior incarnation stopped. Steering closes only after its effect or replacement record is durable. Only the declared reconciliation owner writes product truth; recovery is forbidden during commit-to-close and otherwise uses a stopped-writer barrier.
