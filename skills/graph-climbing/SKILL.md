...
- Resolve `serial-no-ledger`, `serial-ledger`, or `N-worker-epoch` before work. Every ledger lease binds one vertical, envelope hash, unique worker incarnation, and exclusive workspace. Resume exact bindings only. Handoff proves the prior incarnation stopped and preserves or durably transfers local state. Steering closes only after its effect or replacement record is durable. Only the reconciliation owner writes product truth; lease-backed recovery is forbidden during commit-to-close and otherwise requires a stopped-writer barrier.
