...
- Keep worker and operator responsibilities separate. Ledger workers validate one ready compatible candidate—vertical or companion join—plus its envelope and any applicable epoch, claim it atomically with a unique runtime incarnation, re-read exact ownership and bindings, execute, verify, reconcile, and close that exact lease last. A proven ledgerless single writer locally selects one bounded frontier item as its runtime lease. Release owners pre-bind and validate envelopes before Ready. Recovery, handoff, and epoch replacement are operator procedures behind explicit barriers; ordinary workers stop on mismatch rather than reconstructing those protocols from the goal prompt.
