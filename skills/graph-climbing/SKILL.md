...
- Resolve `serial-no-ledger`, `serial-ledger`, or `N-worker-epoch` before work. Every ledger lease ID is single-assignment to one vertical and envelope hash; recovery fences its worker, resolves or invalidates incomplete records, tombstones it, then creates a new ID. Only N-worker leases also bind an epoch. Reconciliation writes truth only while its lease remains active/current, never from a tombstoned record, and closes that exact ledger ID last.
