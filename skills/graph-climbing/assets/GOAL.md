...
5. For N mutating workers, claim only from one immutable run epoch published by its declared central owner before those workers start. It binds the product/graph base, admitted execution-graph revision, candidate IDs, canonical release-envelope hashes, and validation evidence proving every pair that can become simultaneously executable compatible. Incompatible work must be serialized by a real dependency or gate. Workers never publish or replace the epoch.
6. Do not replace a run epoch while any worker for it can claim. A topology, base, scope, authority, candidate-set, or envelope change is a coordinated stop-the-world regraph: durably pause or terminate all epoch workers; reconcile or explicitly withdraw every claimed, active, or sealed lane and companion join; publish the replacement; then resume the unchanged worker goals. If that barrier cannot be proven, keep the old graph or stop.
