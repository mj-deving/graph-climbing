...
12. Reconcile across product authority, evidence, Git, and ledger as one crash-recoverable logical transaction, not a claimed cross-store atomic write. First persist a reconciliation record binding `reconciliation_id`, exact lease ID, vertical ID, envelope hash, run epoch when present, and input/output snapshots. Verify that tuple still matches the single-assignment lease; then commit product truth and evidence and close that exact ledger ID last with the reconciliation ID and commit reference. Envelope or recovery changes use a new lease ID, preventing stale replay from closing the replacement. On resume, replay an incomplete record idempotently. Downstream work stays blocked until all owning surfaces agree.
