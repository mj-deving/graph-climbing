...
2. Before steering or selection, resolve the execution profile and existing work. In an N-worker run, resume the single owned ledger lease only when it binds the current run epoch and certified envelope hash; multiple, mismatched, or pre-epoch leases fail closed. In a proven single-writer ledger run, resume its one exact-ID lease without requiring an epoch; any lease owned by another worker disproves the profile. An abandoned lease may be reassigned only through the ledger's explicit authorized recovery operation; never steal one because a clock or chat appears stale. In a proven ledgerless single-writer run, resume a valid runtime-local lease when present; after context loss reconstruct partial work from product authority, Git, and evidence before selecting again.
...
7. Use ledger priority and stable ID as the deterministic candidate order and atomically claim one exact candidate. In an N-worker run, immediately before and after claim verify the current run-epoch ID and the candidate's canonical envelope hash against the epoch. On mismatch, do not mutate; use authorized release or recovery and stop. If the claim loses a race, refresh and try the next candidate. A proven single-writer ledger run needs no epoch but still uses the exact atomic claim. Never hold two mutating leases, invent custom locks, or continue from a cached frontier.
