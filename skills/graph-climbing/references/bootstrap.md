...
9. Use Git and existing tests as the initial evidence store. Add a Work Graph or ledger only for long runs, durable coordination, external gates, or concurrent writers. In that scaled profile, each vertical names `satisfies`, `depends_on`, ownership, allowed file/read scope, runtime scope when parallel, and external gates. Bind each lease ID once to its stable vertical and envelope hash. Declare one reconciliation owner and exclusive authority workspace; recovery stops every writer for that scope before resolving records, tombstoning, and reissuing.
