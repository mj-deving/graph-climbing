...
10. Before releasing two or more product lanes, set `topology_contract: cohort-v1` and pre-create one companion reconciliation vertical. Give every lane its `reconcile_via` target and return the exact set through the join's `join_for`. Bind exact base, checkout policy, scopes, probes, stop rules, evidence destinations, product-authority write permission, and central owner. That owner publishes one admission epoch containing the candidate IDs, envelope hashes, graph/product base, and validation evidence only after the prior epoch is quiescent. Validate every simultaneously claimable envelope against every active lease; workers do not replace the epoch.
