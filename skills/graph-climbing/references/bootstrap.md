...
9. Use Git and existing tests as the initial evidence store. Add a Work Graph or ledger only for long runs, durable coordination, external gates, or concurrent writers. In that scaled profile, each vertical names `satisfies`, `depends_on`, ownership, file/read/runtime scope, and gates. Bind each lease ID to its stable vertical, envelope hash, unique worker incarnation, and exclusive workspace. Declare one reconciliation owner; handoff proves the old incarnation stopped, while uncertain recovery stops every scope writer before resolving, tombstoning, and reissuing.
