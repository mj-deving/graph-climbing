...
The same task-free goal supports three profiles. Before Ready, the release owner immutably binds each ledger lease ID to vertical, envelope hash, exclusive workspace, and epoch when applicable. Atomic claim binds a unique runtime incarnation as actor/assignee; stable human identity is insufficient. Resume re-verifies all bindings. Handoff proves the old incarnation stopped, then exclusively acquires its workspace or durably transfers and dispositions all Git, filesystem, runtime, and evidence state; uncertainty recovers. Steering closes only after its effect or replacement record is durable. Only the reconciliation owner writes truth. Lease-backed recovery is forbidden during commit-to-close; otherwise stop every authority-scope writer and freeze claims before resolving, tombstoning, and reissuing. A ledgerless worker reconstructs from truth, Git, and evidence.
