...
The same task-free goal supports three explicit profiles: ledgerless single writer, ledger-backed single writer, and N-worker epoch. Every ledger lease ID is single-assignment to one vertical and canonical envelope hash; only an N-worker lease also binds the epoch. Only the declared reconciliation owner in its exclusive authority workspace may write product truth. Recovery is forbidden during commit-to-close and otherwise stops every writer for that scope, freezes claims, resolves Git and incomplete records, tombstones the old lease, and creates a new one. Tombstoned records never write product truth. A ledgerless single writer reconstructs from product truth, Git, and evidence.
