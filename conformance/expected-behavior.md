...
The same task-free goal can be given to N workers. At every start, resume, and post-reconciliation boundary, each worker reconstructs current authorities. With a ledger it first resumes its single owned lease before steering only when epoch and envelope hash match; multiple, pre-epoch, or mismatched leases fail closed and abandoned leases require authorized recovery. A proven ledgerless single writer may resume its runtime-local lease or reconstruct partial work from product truth, Git, and evidence. Lease-local steering may not change base, scope, authority, run epoch, or topology. A worker never needs a new goal after sealing one vertical.
