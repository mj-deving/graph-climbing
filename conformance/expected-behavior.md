...
The same task-free goal supports three profiles. Before Ready, the release owner immutably binds each ledger lease ID to vertical, envelope hash, exclusive workspace, and epoch when applicable. Atomic claim binds a unique runtime incarnation as actor/assignee; stable human identity is insufficient. Resume re-verifies all bindings. Handoff proves the old incarnation stopped, then exclusively acquires its workspace. Otherwise recovery stops every authority-scope writer, freezes claims, resolves and durably imports or dispositions all Git, filesystem, runtime, and evidence state, tombstones the old lease, and issues a new pre-bound lease. Steering closes only after its effect or replacement record is durable. Only the reconciliation owner writes truth. Lease-backed recovery is forbidden during commit-to-close. A ledgerless worker reconstructs from truth, Git, and evidence.
