...
The same task-free goal supports three explicit profiles: ledgerless single writer, ledger-backed single writer, and N-worker epoch. Every ledger lease binds one vertical, envelope hash, unique worker incarnation, and exclusive workspace; only N-worker leases also bind the epoch. Resume requires exact bindings. Handoff proves the old incarnation stopped and no reconciliation is in flight, then exclusively acquires its original workspace or durably transfers and dispositions all Git, filesystem, runtime, and evidence state; missing or uncertain state enters recovery. Steering closes only after its effect or replacement barrier record is durable. Only the reconciliation owner writes truth. Lease-backed recovery is forbidden during commit-to-close and otherwise stops every scope writer before resolving, tombstoning, and reissuing. A ledgerless worker reconstructs from truth, Git, and evidence.
