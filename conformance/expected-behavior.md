...
The same task-free goal supports three explicit profiles: ledgerless single writer, ledger-backed single writer, and N-worker epoch. Every ledger lease ID is single-assignment to one vertical and canonical envelope hash; only an N-worker lease also binds the epoch. Only the declared reconciliation owner in its exclusive authority workspace may write product truth. In lease-backed profiles, recovery is forbidden during commit-to-close and otherwise stops every scope writer, freezes claims, resolves Git and records, tombstones the old lease, and creates a new one. Records replay idempotently only while their bound lease remains active/current; tombstoned records never write product truth. A ledgerless single writer recovers by reconstructing from product truth, Git, and evidence.
