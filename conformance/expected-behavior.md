...
The same task-free goal supports three profiles. Before Ready, the release owner immutably binds each ledger lease ID to vertical, envelope hash, exclusive workspace, applicable epoch, and one authority-scope recovery barrier ID. Atomic claim binds a unique runtime incarnation; stable human identity is insufficient. Resume re-verifies all bindings. Handoff proves the old incarnation stopped, no reconciliation is in flight, then exclusively acquires its workspace. Otherwise atomically claim the recovery barrier; losers stop. Its owner freezes claims, stops scope writers, and first replays or dispositions incomplete records on their active leases. Replay closes without replacement. Only if work remains does it import or disposition all state, tombstone, and issue a new pre-bound lease plus successor barrier; close the barrier last. Lease-backed recovery is forbidden during a live commit-to-close. A ledgerless worker reconstructs from truth, Git, and evidence.
