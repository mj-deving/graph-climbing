...
The same task-free goal supports three explicit profiles: ledgerless single writer, ledger-backed single writer, and N-worker epoch. Each reconstructs current authorities before steering. Only an N-worker ledger lease must match epoch and envelope hash; a serial ledger lease resumes by exact ID and any foreign owner disproves single-writer status. Multiple or mismatched N-worker leases fail closed and abandonment requires authorized recovery. A ledgerless single writer resumes its runtime-local lease or reconstructs partial work from product truth, Git, and evidence. Lease-local steering may not change base, scope, authority, run epoch, or topology.
