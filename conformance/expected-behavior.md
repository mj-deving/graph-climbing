...
The same task-free goal supports three explicit profiles: ledgerless single writer, ledger-backed single writer, and N-worker epoch. Every ledger lease ID is single-assignment to one vertical and canonical envelope hash; only an N-worker lease also binds the epoch. A serial lease resumes by exact ID plus hash, and foreign ownership or a lost race disproves its profile. Recovery fences the old worker, tombstones that lease ID, and creates a new one. Reconciliation binds lease ID, vertical, hash, and epoch, then closes that exact ID last; stale replay cannot close a replacement. A ledgerless single writer reconstructs from product truth, Git, and evidence.
