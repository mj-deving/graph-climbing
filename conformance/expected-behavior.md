...
The same task-free goal supports three profiles. Before Ready, the release owner immutably binds each ledger lease ID to vertical, envelope hash, exclusive workspace, and epoch when applicable. Atomic claim binds a unique runtime incarnation as actor/assignee; stable human identity is insufficient. Resume re-verifies all bindings. Handoff proves the old incarnation stopped, no reconciliation is in flight, then exclusively acquires its workspace. Otherwise recovery stops every authority-scope writer, freezes claims, and first replays or dispositions incomplete reconciliation records on their active leases. Successful replay closes without replacement. Only if work remains does recovery resolve and import or disposition all state, tombstone the old lease, and issue a new pre-bound lease. Steering closes only after its effect or replacement record is durable. Only the reconciliation owner writes truth. Lease-backed recovery is forbidden during commit-to-close. A ledgerless worker reconstructs from truth, Git, and evidence.

Before initial N-worker start or replacement, all mutating workers are durably paused or terminated and every prior lease, seal, reservation, and join is reconciled or withdrawn. One central owner then publishes an immutable run epoch containing the graph revision, canonical envelope hashes, and proof that every pair whose reservation lifetimes can overlap is compatible. A reservation spans claim through direct reconciliation or cohort join. Workers verify epoch and hash before and after claim; mismatch permits no mutation. The same goals resume. A proven single writer may select claim-first work without a ledger; N workers may not.
