...
Adoption/bootstrap also persists the resolved authority map in repository-native agent instructions. The repository-root `AGENTS.md` is merged, never replaced, or created when absent; nested instruction files remain scoped overlays. `CLAUDE.md` is touched only when used or requested. The stable adapter names exact product authority, optional ledger, evidence, checker, resume, ownership, verification, reconciliation, and approval boundaries. It contains no mutable task IDs, owners, progress, PR matrices, or dated hot context. When `.beads/` exists without a competing tracker, the adapter explicitly maps the operational ledger to Beads. Audit-only use writes nothing.
...
The installed worker text is identical, task-free, and below the native 4,000-character goal limit. It contains the repeated worker loop and stop boundaries, not a natural-language implementation of the control plane. Every executable item has a complete immutable envelope. A ledger worker reconstructs durable state, derives the frontier, first resumes a lease already owned by its runtime incarnation after validating exact ownership, binding, any applicable epoch, and inactive barriers; otherwise it validates and claims one compatible ready candidate—vertical or companion join—under the same controls, then re-reads exact ownership, bindings, and barriers. A ledgerless worker proceeds only as a proven single writer and locally selects one bounded enveloped frontier item as its runtime lease. At start or resume, both profiles process all ready steering for that lease before execution or blocker re-evaluation; later safe boundaries repeat the ingress. Both profiles execute and verify the envelope, reconcile, close any ledger lease last, and repeat. Steering targets the owned lease or proven ledgerless writer, is atomically claimed when ledger-backed, and closes only after its effect or disposition is durable.
