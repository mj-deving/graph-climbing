...
The same task-free goal supports three profiles. Before Ready, the release owner immutably binds each ledger lease ID to vertical, envelope hash, exclusive workspace, and epoch when applicable. Atomic claim binds a unique runtime incarnation as actor/assignee; stable human identity is insufficient. Resume re-verifies all bindings. Handoff proves the old incarnation stopped and preserves or dispositions all workspace state; uncertainty recovers. Steering closes only after its effect or replacement record is durable. Only the reconciliation owner writes truth; lease-backed recovery is forbidden during commit-to-close. A ledgerless worker reconstructs from truth, Git, and evidence.
