...
The same task-free goal supports three explicit profiles: ledgerless single writer, ledger-backed single writer, and N-worker epoch. Every ledger lease ID is single-assignment to one vertical and canonical envelope hash; only an N-worker lease also binds the epoch. Recovery fences the old worker, resolves or invalidates its incomplete reconciliation records, tombstones that lease ID, and creates a new one. Reconciliation writes product truth only when the bound lease remains active/current, then closes that exact ID last. Tombstoned records are superseded and never replayed into product truth. A ledgerless single writer reconstructs from product truth, Git, and evidence.
