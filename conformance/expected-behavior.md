...
The same task-free goal can be given to N workers. At every start, resume, and post-reconciliation boundary, each worker reconstructs current authorities and first resumes its single owned lease before steering only when epoch and envelope hash match. Multiple, pre-epoch, or mismatched leases fail closed; abandoned leases require explicit authorized recovery. Lease-local steering may not change base, scope, authority, run epoch, or topology. With no lease, ledger priority and stable ID determine candidate order and an exact atomic claim grants one mutating lease. A worker never needs a new goal after sealing one vertical.
