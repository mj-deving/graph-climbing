...
The installed worker text is identical, task-free, and at most 1,100 characters. It contains the repeated worker loop and stop boundaries, not a natural-language implementation of the control plane. The worker reconstructs durable state, derives the frontier, claims at most one compatible ready lease, re-reads its envelope, executes and verifies it, reconciles, closes the exact lease last, and repeats. A ledgerless worker proceeds only as a proven single writer.
