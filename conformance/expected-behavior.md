...
The same task-free goal supports three explicit profiles: ledgerless single writer, ledger-backed single writer, and N-worker epoch. Every ledger lease binds one vertical, envelope hash, worker incarnation, and workspace; only N-worker leases also bind the epoch. Resume requires exact bindings. Handoff proves the old incarnation stopped and no reconciliation is in flight, then exclusively acquires its original workspace or durably transfers/dispositions all Git, filesystem, runtime, and evidence state; uncertainty enters recovery. Steering closes only after its effect or replacement barrier record is durable. Only the declared reconciliation owner writes truth. A ledgerless worker reconstructs from truth, Git, and evidence.
