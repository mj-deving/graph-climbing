...
The same task-free goal supports three explicit profiles: ledgerless single writer, ledger-backed single writer, and N-worker epoch. Each reconstructs current authorities before steering. Every ledger lease binds a canonical envelope hash; only an N-worker lease also binds the epoch. A serial ledger lease resumes by exact ID plus hash, and any foreign owner or lost claim race disproves single-writer status. N-worker claim losers may refresh. Multiple or mismatched leases fail closed and abandonment requires authorized recovery. A ledgerless single writer resumes locally or reconstructs from product truth, Git, and evidence.
