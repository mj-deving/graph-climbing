...
The bundled checker parses only the strict reference `SPEC.md` shape. A repository-native ISA remains the product authority and workers read it semantically; a generic prose-to-graph parser is not a prerequisite. Use a repository-native checker or adapter only when the repository explicitly provides one over machine-readable fields. Any projection remains disposable derived data, while execution ownership and scopes come from the ledger-bound envelope. Never rewrite or duplicate product claims merely to satisfy `graph-check`.
