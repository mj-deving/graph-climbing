...
2. Before steering or selection, resolve leases already owned by this worker. Resume the single valid owned lease only when it binds the current run epoch and certified envelope hash. Multiple owned mutating leases or a pre-epoch lease block normal execution. An abandoned lease may be reassigned only through the ledger's explicit authorized recovery operation; never steal one because a clock or chat appears stale.
