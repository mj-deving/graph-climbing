...
2. Before steering or selection, resolve leases already owned by this worker. Resume the single valid owned lease instead of selecting another. Multiple owned mutating leases are an invariant violation. An abandoned lease may be reassigned only through the ledger's explicit authorized recovery operation; never steal one because a clock or chat appears stale.
3. With an owned lease, ingest only steering explicitly targeting that lease at a declared safe boundary. Claim and close the steering record immediately. If it would change the lease's base, scope, authority, or admission epoch, stop and route it through central withdrawal or reconciliation. Without an owned lease, disposition ready steering before deriving new product work.
...
5. For N mutating workers, claim only from one durable current admission epoch. Its declared central reconciliation owner publishes the epoch with graph/product base, candidate IDs, immutable release-envelope hashes, and validation evidence proving every candidate pair compatible with all active leases. Workers never publish or replace epochs. Incompatible work must be serialized by a real dependency or gate.
6. Replace an admission epoch only when the prior epoch is quiescent: it has no claimed, active, or sealed lane and every companion join is reconciled or explicitly withdrawn. Before and immediately after claiming, verify that the candidate names the same current epoch. On mismatch, do not mutate; use authorized release or recovery and stop. Reject unresolved authority, base drift, scope collision, shared mutable runtime, unmet predecessors or gates, or missing companion reconciliation.
7. Use ledger priority and stable ID as the deterministic candidate order. Atomically claim exactly one admitted vertical. If the claim loses a race, refresh durable state and try the next candidate. Never hold two mutating leases, invent custom locks, or continue from a cached frontier.
...
- immutable current admission-epoch ID, envelope hash, central owner, and validation evidence when N workers may claim concurrently;
