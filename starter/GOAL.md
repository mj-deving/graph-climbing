...
2. Before steering or selection, resolve the execution profile and existing work. Every ledger lease ID is single-assignment to one stable vertical ID and one canonical release-envelope hash. In an N-worker run, resume the single owned lease only when both run epoch and hash match; multiple, mismatched, or pre-epoch leases fail closed. In a proven single-writer ledger run, resume its exact lease ID only when the hash matches, without requiring an epoch; any foreign owner disproves the profile. Authorized recovery first fences the old worker and resolves or invalidates every incomplete reconciliation record for its lease, then tombstones that lease ID and creates a new one; never reassign or reuse an old lease ID because a clock or chat appears stale. In a proven ledgerless single-writer run, resume a valid runtime-local lease when present; after context loss reconstruct partial work from product authority, Git, and evidence before selecting again.
...
7. Use ledger priority and stable ID as the deterministic candidate order and atomically claim one exact candidate while binding its canonical envelope hash. In an N-worker run, immediately before and after claim verify run epoch and hash against the epoch; on a lost race, refresh and try the next candidate. In a proven single-writer ledger run, verify the bound hash without an epoch; a lost race disproves single-writer status and must stop for reclassification. On any mismatch, do not mutate; use authorized release or recovery. Never hold two mutating leases, invent custom locks, or continue from a cached frontier.
...
12. Reconcile across product authority, evidence, Git, and ledger as one crash-recoverable logical transaction, not a claimed cross-store atomic write. First persist a record binding `reconciliation_id`, exact lease ID, vertical ID, envelope hash, run epoch when present, and input/output snapshots. Immediately before committing, verify that the tuple matches and this lease is still the active current lease for the vertical, not tombstoned or replaced. Then commit product truth and evidence and close that exact ledger ID last with the reconciliation ID and commit reference. Replay only active-lease records idempotently; disposition tombstoned records as superseded without writing product truth. Downstream work stays blocked until all owning surfaces agree.
